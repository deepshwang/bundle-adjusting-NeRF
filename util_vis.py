...
def get_pose_coord_mesh(pose,depth=1):
    vertices = torch.tensor([[1., 0., 0.],           # x-axis
                             [0., 1., 0.],           # y-axis
                             [0., 0., 1.],
                             [0., 0., 0.]]) * depth

    vertices = camera.cam2world(vertices[None], pose)
    wireframe = vertices[:, [3, 0, 3, 1, 3, 2]]
...
def plot_object_poses_as_coordinates(fig, pose, step):
    # get the camera meshes
    ...
    setup_3D_plot(ax1, elev=-90, azim=-90, lim=edict(x=(-1 * t, 1 * t), y=(-1 * t, 1 * t), z=(-1 * t, 1 * t)))
    setup_3D_plot(ax2, elev=0, azim=-90, lim=edict(x=(-1 * t, 1 * t), y=(-1 * t, 1 * t), z=(-1 * t, 1 * t)))
    ax1.set_title("forward-facing view", pad=0)
    ax2.set_title("top-down view", pad=0)
